...
public class RestAssuredTests {
  ...
  private Issue getIssue(int id) throws IOException {
    String json = RestAssured.get("https://bugify.stqa.ru/api/issues/" +
            Integer.toString(id) + ".json?attachments=false&comments=false&followers=false&history=false").asString();

    JsonElement parsed = new JsonParser().parse(json);
    JsonElement issues = parsed.getAsJsonObject().get("issues");
    Set<Issue> oldIssues = new Gson().fromJson(issues, new TypeToken<Set<Issue>>(){}.getType());

    return oldIssues.stream().findFirst().get();
  }
